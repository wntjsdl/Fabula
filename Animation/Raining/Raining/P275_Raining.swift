...
//  Raining
//
//  Created by JuSun Kang on 2/21/24.
//

import SwiftUI

struct P275_Raining: View {
    public init(){}
...
fileprivate struct Raining: View {
    ...
    private func addRaindrop() {
        ...
        let raindrop = FallingRaindrop(scale: scale, animationDuration: fallDuration)
        
        raindrops.append(FallingRaindrops(id: newRaindropId, raindrop: raindrop))
    ...
    var body: some View {
        GeometryReader { screen in
            Raindrop()
                ...
                .offset(x: screen.size.width * offsetXRatio, y: screen.size.height * offsetYRatio)
                .onAppear {
                    withAnimation {
...
fileprivate struct Raindrop: View {
    var body: some View {
        RaindropShape()
            .fill(LinearGradient(colors: [.blue.opacity(0.8)], startPoint: .top, endPoint: .bottom))
    }
}
...
fileprivate struct RaindropShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        path.addCurve(to: CGPoint(x: 0, y: raindropBottomStart), control1: CGPoint(x: midX - midX / 4, y: rect.height / 4), control2: CGPoint(x: 0, y: midY))
        
        path.addQuadCurve(to: CGPoint(x: midX, y: rect.height), control: CGPoint(x: 0, y: rect.height))
        
        path.addQuadCurve(to: CGPoint(x: rect.width, y: raindropBottomStart), control: CGPoint(x: rect.width, y: rect.height))
        
        path.addCurve(to: CGPoint(x: midX, y: 0), control1: CGPoint(x: rect.width, y: midY), control2: CGPoint(x: midX + midY / 4, y: rect.height / 4))
