...
//  ContentView.swift
//  Rotating_Playing_Card
//
//  Created by JuSun Kang on 2/13/24.
//
...
fileprivate struct FlippingAnimation: Animatable, ViewModifier {
    ...
    func body(content: Content) -> some View {
        DispatchQueue.main.async {
            if 90 <= self.angle && self.angle <= 270 { self.displaySide = .front } else { self.displaySide = .back }
        }
...
fileprivate struct CardFront: View {
    private let frontColor = [Color.orange, .pink, .purple.opacity(0.7)].randomElement()!
...
fileprivate struct DiamondFill: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        for loc in stride(from: 10.0, through: traveling, by: 18) {
            ...
            let forwardXEnd = loc <= rect.height ? 0 : loc - rect.width
            let forwardYEnd = loc <= rect.height ? loc : rect.width
...
fileprivate struct RandomShape: Shape {
    ...
    private lazy var rectengle: RoundedRectangle = { RoundedRectangle(cornerRadius: 20) }()
...
fileprivate struct Star: Shape {
    ...
    var animatableData: AnimatablePair<CGFloat, CGFloat> {
        get { AnimatableData(count, innerRatio) }
        set {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        let correnction = count != round(count) ? 1 : 0
        for corner in 0..<totalPoints + correnction {
...
fileprivate struct RaindropShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        path.addCurve(to: CGPoint(x: 0, y: raindropBottomStart), control1: CGPoint(x: midX - midY / 4, y: rect.height / 4), control2: CGPoint(x: 0, y: midY))
        
        path.addQuadCurve(to: CGPoint(x: midX, y: rect.height), control: CGPoint(x: 0, y: midY))
        
        path.addQuadCurve(to: CGPoint(x: rect.width, y: raindropBottomStart), control: CGPoint(x: rect.width, y: rect.height))
        
        path.addCurve(to: CGPoint(x: midX, y: 0), control1: CGPoint(x: rect.width, y: midY), control2: CGPoint(x: midX + midX / 4, y: rect.height / 4))
